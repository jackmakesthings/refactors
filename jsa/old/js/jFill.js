...
(function($) {
  ...
  $.fn.jFill = function(options) {
    var i,
      ...
      // }}}
      // {{{ create_slide(src, deferred)
      create_slide = function(src, deferred){
        var new_slide, 
          sizes, 
          container_dims;
      ...
      // }}}
      // {{{ create_slides(srcs, lazy_load)
      create_slides = function(srcs, lazy_load){
        var i, 
          deferred, 
          make_deferred = function(src, deferred) {
            deferred = deferred || false;
        ...
        for (i in srcs) {
          if (srcs.hasOwnProperty(i)) {
            if (lazy_load) {
              if (is_first) {
                // deferred = $.Deferred().done(create_slide(srcs[i]));
                deferred = make_deferred(srcs[i]);
                is_first = false;
              } else {
                // deferred = $.Deferred().done(create_slide(srcs[i], deferred));
                deferred = make_deferred(srcs[i], deferred);
              }
            } else {
              create_slide(srcs[i]);
            }
    ...
    if (typeof options === 'string') {
      ...
      if (options.hasOwnProperty('images') && $.type(options.images) === 'array') {
        create_slides(options.images, options.hasOwnProperty('lazy_load'));
        // {{{ add event bindings
        ...
        if (options.hasOwnProperty('image_load_callback')) {
          image_load_callback = options.image_load_callback;
        }
        // }}}
      } else {
