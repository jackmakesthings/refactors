$(function(){
    ...
    var contact_form = $('#contact-us'),
        ...
        search_input = $('.search_input', search_form)
            .data('loaded', false),
        // product displays
        ...
        filter_tag = $('<li class="filter"><a href="javascript:;"><span>x</span><label></label></a></li>')
            .on({
        ...
        d_overall_height_slider = $('div#overallHeightSlider', product_sorting),
        ...
        submit: function(e){
            ...
            if(search_input.val().length >2){
                $.post( '/rpc/autocomplete/user-query/',
                    { proceed : true, query : search_input.val() },
                    function(data){
                        if(data.success){
...
// {{{ pagination and filters
            ...
            pajax = $.post('/rpc/paginator/'+el_page+'/',
                ...
                function(data){
                    if(data.success){
                        ...
                        if(get_bounds){
                            length_slider.rangeSlider("option", "bounds", { min: parse_round(data.bounds.min_depth), max: parse_round(data.bounds.max_depth, true)});
                            width_slider.rangeSlider("option", "bounds", { min: parse_round(data.bounds.min_width), max: parse_round(data.bounds.max_width, true)});
                            height_slider.rangeSlider("option", "bounds", { min: parse_round(data.bounds.min_height), max: parse_round(data.bounds.max_height, true)});

                            diameter_slider.rangeSlider("option", "bounds", { min: parse_round(data.bounds.min_diameter), max: parse_round(data.bounds.max_diameter, true)});
                            diameter_height_slider.rangeSlider("option", "bounds", { min: parse_round(data.bounds.min_diameter_height), max: parse_round(data.bounds.max_diameter_height, true)});
                            d_overall_height_slider.rangeSlider("option", "bounds", { min: parse_round(data.bounds.min_diameter_overall_height), max: parse_round(data.bounds.max_diameter_overall_height,true)});
    ...
    function filter_sizes(){
        ...
        if($(this).parent('form').attr('id') === 'diameter'){
